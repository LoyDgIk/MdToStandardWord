...
- 正文、表格单元格、表内注、图内段、来源和脚注中的数学变量需要和公式字体一致时，统一使用行内公式 `$$...$$`，如 `$$T_r$$`、`$$Q_e$$`、`80%$$v_{\text{max}}$$`。不要用行内公式写长句或手写公式编号。
...
表格中的普通注写在相关单元格内，使用 `〔注：...〕`；单元格可以只有注，例如 `<td colspan="3">〔注：...〕</td>`。注内公式同样使用 `$$...$$`。同一单元格连续多个 `〔注：...〕` 会自动生成多条编号注，单条注使用不编号的注样式。需要在表格单元格中标出脚注引用点时，使用 `〔脚注〕`；脚注说明使用块级 `{脚注} ...`，编号由 `标准文件_图表脚注` 样式自动生成，不在 Markdown 中手写 a、b、c。
